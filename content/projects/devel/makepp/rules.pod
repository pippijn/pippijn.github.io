...
C<Autocomb> defines a few rules itself and has knowledge about automake-defined
rules, so that e.g. rules for compiling C sources are not emitted again. The
rules feature in C<Autocomb> is the reason this project was actually started. The
...
The introductory example is already a pattern rule. Patterns in C<Autocomb> are
...
C<@>. C<Autocomb> extends this to automake C<silent-rules>.
...
If C<Autocomb> is not flexible enough for the use case, external rules may be
...
C<parallel-tests> facilities. These tools are the reason C<Autocomb> targets automake
...
lower level than C<Autocomb>, so it is not advisable to override many of its rules.
